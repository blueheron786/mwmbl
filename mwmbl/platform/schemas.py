...
class RejectionFieldsMixin:
    ...
    def _check_decision_fields(self):
        ...
        if reason == "OTHER" and not detail.strip():
            raise ValueError(
                "rejection_detail is required when rejection_reason is OTHER - it is what the submitter is shown"
            )
        return self
...
class UpdateDomainSubmission(RejectionFieldsMixin, ModelSchema):
    ...
    suggested_status: Optional[str] = Field(
        default=None, max_length=20, description="The action the tool suggested, as shown to the moderator."
    )
    suggested_reason: Optional[str] = Field(
        default=None, max_length=20, description="The rejection reason the tool suggested, if any."
    )
    ...
    suggestion_model_version: Optional[str] = Field(
        default=None, max_length=50, description="Version of the model that produced the shown suggestion."
    )
...
class QueueItemSchema(Schema):
    ...
    name: str
    submission_count: int = Field(description="How many submissions of this domain are still pending.")
    first_submitted_on: datetime = Field(description="When this domain was first submitted, by whoever asked first.")
    first_submitted_by: int
    first_submitted_by_username: str
    upvotes: int = Field(description="Votes cast on results from this domain, ignoring a leading `www.`.")
    downvotes: int
    ...
    suggestion: Optional[SuggestionSchema] = Field(
        default=None, description="Absent until the domain has been crawled. Never a placeholder."
    )
...
class SubmissionDetailSchema(Schema):
    ...
    prior_decisions: dict = Field(default={}, description="Counts of earlier decisions on this same domain.")
    index_stats: dict = Field(default={}, description="What the crawler already knows about this domain.")
...
class VoteRequest(Schema):
    ...
    url: str = Field(description="The URL of the search result being voted on", example="https://example.com/article")
    query: str = Field(description="The search query that returned this result", example="python tutorial")
...
class VoteRemoveRequest(Schema):
    ...
    url: str = Field(
        description="The URL of the search result to remove the vote from", example="https://example.com/article"
    )
    query: str = Field(description="The search query that returned this result", example="python tutorial")
...
class VoteStats(Schema):
    ...
    upvotes: int = Field(description="Total number of upvotes for this search result", example=15)
    downvotes: int = Field(description="Total number of downvotes for this search result", example=3)
    user_vote: Optional[Literal["upvote", "downvote"]] = Field(
        default=None, description="The current user's vote on this result, if any", example="upvote"
    )
...
class VoteStatsRequest(Schema):
    ...
    query: str = Field(description="The search query that returned these results", example="python tutorial")
    urls: list[str] = Field(
...
class VoteResponse(Schema):
    ...
    votes: dict[str, VoteStats] = Field(
        ...
        example={
            "https://example.com/article": {"upvotes": 15, "downvotes": 3, "user_vote": "upvote"},
            "https://another-site.com/page": {"upvotes": 8, "downvotes": 1, "user_vote": None},
        },
    )
...
class UserVoteHistory(Schema):
    ...
    url: str = Field(
        description="The URL of the search result that was voted on", example="https://example.com/article"
    )
    query: str = Field(description="The search query that returned this result", example="python tutorial")
    vote_type: Literal["upvote", "downvote"] = Field(description="The type of vote cast by the user", example="upvote")
    timestamp: datetime = Field(description="When the vote was cast", example="2024-01-15T10:30:00Z")
